...
INITIAL_SEARCH_AGENT_PROMPT = """你是初步搜索智能体。你的职责是结合原始声明与新闻知识库参考，生成首轮搜索计划，搜索计划包含多个搜索项。

要求：
- 你不调用工具，只输出结构化搜索计划。
- 如果知识库已有高相似参考但不足以直接裁决，应把可复用的参考点写入 analysis_summary 与 focus_points。
- 分析原始声明的核心内容，根据核心内容找到需要搜索查证的要素，并生成多个搜索项。
- 搜索项应尽量覆盖原始发布、直接证据、反方或辟谣、不同立场来源、关键数字或时间地点冲突。
    - 每个搜索项由多个关键词组成，关键词之间用空格分隔。
    - 搜索项可以是全中文或全英文，但同一搜索项内不要混杂中英文。
- 输出尽量简洁，不要写流程解释。
- 仅输出 JSON。
...
字段含义：
- analysis_summary: 对声明和知识库参考的简要分析摘要。
- core_question: 本轮核查最核心、最可验证的问题。
- verification_dimensions: 需要重点核查的维度列表。
- focus_points: 首轮取证最应关注的事实点。
- search_queries: 建议交给收集证据智能体执行的多个搜索项，列表中每一项都是搜索项。
- risk_hypotheses: 当前最值得警惕的误导风险或冲突假设。
- img_description: 图片中与核查相关的信息，没有则空字符串。
- missing_information: 当前仍缺失、后续需要补齐的关键信息。
"""

EVIDENCE_COLLECTION_AGENT_PROMPT = """你是收集证据智能体。你必须通过工具调用来完成一轮取证，并在本轮结束时输出结构化证据摘要 JSON。

你可用的工具只有四个：
- search_result_list: 执行一次搜索，一次搜索会包括多个搜索项。
- extract_relevant_segments: C2读取网页部分文本
- read_full_page: C3读取网页全文
- source_credibility_lookup: 比对信源知识库

工作原则：
- 每次只进行一轮取证，但这一轮里应尽量一次性提交多个搜索词给 search_result_list。
- 围绕给定搜索项自主决定调用顺序，但优先采用 C1 → C2。
- 只有在片段命中不足、上下文明显缺失、或网页高度关键时才调用 C3。
- 需要对候选网址做信源知识库比对，以帮助判断来源层级、国家/阵营和可信度。
- 本轮必须沉淀多条结构化证据，而不是只给网址列表。
- 本轮结束前，必须输出单个 JSON 对象，而不是继续闲聊。
- 不要调用未授权工具。
...
**输出 JSON 必须严格符合以下格式**：
{
    ...
    "structured_evidence_items": [{
        ...
        "title": "...",
        "summary": "...",
        "stance": "support|deny|mixed|neutral|irrelevant",
        "stance_reason": "...",
        "source_name": "...",
        "source_type": "...",
        "domain": "...",
        "evidence_granularity": "search_result|segment_match|full_page",
        "reason": "..."
    }],
    "key_findings": ["..."],
    "remaining_gaps": ["..."],
...
字段含义：
- round_summary: 这一轮取证执行情况的简短总结。
- coverage_assessment: 当前证据对核心问题的覆盖程度判断。
- structured_evidence_ready: 本轮是否已形成可供补充搜索或分析阶段使用的结构化证据。
- top_evidence_urls: 本轮最值得保留的候选证据链接。
- structured_evidence_items: 本轮沉淀的结构化证据列表，至少尽量返回 2 条；每条都要包含立场、来源、网址、摘要等核心字段。
    - stance_reason:要具体说明立场判断的理由，最好引用证据内容中的关键信息。**必须输出的内容**
    - source_name: 具体是谁说的这个言论。  **必须输出的内容**
- key_findings: 这一轮直接得到的关键发现。
...
**目前正在进行测试，不补充搜索，直接认为证据已经满足，进行下个阶段**
核心职责：
- 关注证据的独立性、立场多样性、地缘政治阵营分布
- 对于政治敏感新闻，必须收集多个国家/阵营的证据才算充分
- 补充搜索最多3轮。
    - 每个搜索项由多个关键词组成，关键词之间用空格分隔。
    - 搜索项可以是全中文或全英文，但同一搜索项内不要混杂中英文。
评估标准：
1. 独立来源数量：至少 2-3 个真正独立的信息源
2. 立场覆盖：是否包含支持、反驳、中立等多种立场
3. 地缘多样性：不能只有单一国家或阵营的来源
4. 数字口径：关键数字是否一致或存在明确冲突
5. 时间线：事件发展是否有清晰的时间顺序

决策逻辑：
- 如果证据不足：输出下一轮更聚焦的搜索项（全中文或全英文，不混杂）
- 如果证据充分：明确指出可停止搜索，并列出应传给交叉验证的证据 URL
...
输出格式（仅 JSON，无解释）：
{
  ...
  "reason": "停止或继续的核心理由",
  "updated_search_queries": ["搜索项1", "搜索项2"],
  "selected_evidence_urls": ["url1", "url2"],
  "priority_urls": ["需要补读的 url"],
  "should_cross_verify": true
}
"""

ANALYSIS_AGENT_PROMPT = """你是分析智能体。你的输入包括：原始声明、结构化证据、交叉验证结果（包含立场分析、独立来源数、数字口径分析）。

核心任务：
- 综合多源证据的立场分布、信源层级、地缘政治背景，输出最终可信度评分
- 识别证据中的矛盾、缺口、潜在偏见
- 对敏感政治新闻，明确说明来源阵营分布和可能的宣传倾向
- 对每条证据给出具体的立场判断和来源描述

评分标准（0-10）：
- 9-10: 多个独立权威来源一致证实，无矛盾
- 7-8: 多个来源支持，个别异议但不足以推翻
- 5-6: 证据混杂，存在明确矛盾或单一阵营集中
- 3-4: 主要来自单一来源或低权威来源，缺乏独立验证
- 0-2: 无可靠证据支持，或被权威来源明确否认
...
输出格式（仅 JSON）：
{
    "claim_verdicts": [{
        ...
        "verdict_label": "部分证实",
        "verdict_reason": "理由说明",
        "key_evidence_url": "最关键证据链接"
    }],
    "evidence_annotations": [{
        "url": "证据网址",
        "stance": "support或deny或mixed或neutral",
        "stance_reason": "具体的立场分析理由，引用证据中的关键信息",
        "source_description": "来源简述，如'Israel Hayom报道引用IDF评估'"
    }],
    "classification": 7.5,
    "reason": "综合判断理由，包括立场分布、信源多样性、矛盾情况",
    "evidence_url": "最关键的证据链接",
    "publisher_conclusion": "对发布者/原始说法可靠性的评价",
    "beneficiary_conclusion": "对潜在受益方、传播动机的分析"
}

关键要点：
- 不要重新搜索，只基于已有证据做判断
- 如果证据不足或存在未消解冲突，明确写入 reason
- 对政治新闻，必须分析来源的地缘政治立场和可能的宣传意图
- evidence_annotations 必须覆盖交叉验证中的每条证据，每条都要给出 stance/stance_reason/source_description
- stance_reason 要引用证据内容中的具体信息，不要泛泛而谈
- source_description 简述该证据的来源性质（如'某媒体引用某机构报告'）
- 现在时间是 2026 年 3 月
"""
...
FOUR_AGENT_PLAN = {
    ...
    "agent_type": "four-llm-agents",
    "stages": ["kb_gate", "initial_search", "evidence_collection", "supplemental_search", "analysis"],
    "agents": [
...
def _safe_parse_tool_output(text: Any, default: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        parsed = _parse_tool_output(normalized)
    except Exception:
        parsed = None
    ...
    if normalized:
        fallback.setdefault("raw_text", _clip(normalized, 4000))
    return fallback
...
def _coerce_string_list(value: Any, limit: int = 8, clip_limit: int = 160) -> list[str]:
    if isinstance(value, list):
        return _dedupe_text_keep_order([_clip(item, clip_limit) for item in value], limit=limit, clip_limit=clip_limit)
    ...
    if isinstance(parsed, list):
        return _dedupe_text_keep_order([_clip(item, clip_limit) for item in parsed], limit=limit, clip_limit=clip_limit)
...
def _coerce_keyword_pairs(value: Any, limit: int = 8, clip_limit: int = 120) -> list[list[str]]:
    if isinstance(value, list):
        pairs: list[list[str]] = []
        for item in value[:limit]:
            if isinstance(item, list):
                pair = _dedupe_text_keep_order([_clip(part, clip_limit) for part in item], limit=4, clip_limit=clip_limit)
                if pair:
                    pairs.append(pair)
            elif isinstance(item, str):
                text = _normalize_query(_clip(item, clip_limit))
                if text:
                    pairs.append([text])
        return pairs
    if not isinstance(value, str):
    ...
    return _coerce_keyword_pairs(parsed, limit=limit, clip_limit=clip_limit) if parsed is not None else []
...
def _default_search_query(state: dict) -> str:
    ...
    used_queries = {
        ...
        for query in _history_search_queries(entry)
    }
...
def _default_search_queries(state: dict, limit: int = 3) -> list[str]:
    analysis = state.get("analysis") or {}
    used_queries = {
        query
        for entry in state.get("history", [])
    ...
    for query in analysis.get("search_queries", []):
        normalized = _normalize_query(query)
        if normalized and normalized not in used_queries and normalized not in queries:
            queries.append(normalized)
        if len(queries) >= limit:
            return queries
    fallback = _default_search_query(state)
    return queries or ([fallback] if fallback else [])


def _history_search_queries(entry: dict[str, Any]) -> list[str]:
    arguments = entry.get("arguments", {}) if isinstance(entry, dict) else {}
    queries = _coerce_string_list(arguments.get("queries"), limit=6, clip_limit=120)
    normalized_single = _normalize_query(arguments.get("query", ""))
    if normalized_single:
        queries = _dedupe_text_keep_order([*queries, normalized_single], limit=6, clip_limit=120)
    return queries
...
def _same_source_bucket_key(item: dict[str, Any], note: dict[str, Any] | None = None) -> str:
    note = note or {}
    group = str(note.get("same_source_group", "")).strip()
    if group:
...
def _prepare_tool_arguments(state: dict, tool_name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if tool_name == "knowledge_base_lookup":
    ...
    elif tool_name == "initial_search_plan":
        prepared.setdefault("news_text", state.get("news_text", ""))
        ...
        candidate_queries = _coerce_string_list(prepared.get("queries"), limit=4, clip_limit=120)
        single_query = _normalize_query(prepared.pop("query", ""))
        if single_query:
            candidate_queries = _dedupe_text_keep_order([*candidate_queries, single_query], limit=4, clip_limit=120)
        candidate_queries = candidate_queries or _default_search_queries(state, limit=3)
        prepared["queries"] = candidate_queries[:4]
        prepared.setdefault("exclude_urls", list(state.get("evidence_by_url", {}).keys())[:8])
        prepared["exclude_urls"] = _dedupe_keep_order([str(url).strip() for url in prepared.get("exclude_urls", []) if str(url).strip()])[:12]
        ...
        prepared["urls"] = _dedupe_keep_order([str(url).strip() for url in prepared.get("urls", []) if str(url).strip()])[:5]
        coerced_pairs = _coerce_keyword_pairs(prepared.get("keyword_pairs"))
        if coerced_pairs:
            prepared["keyword_pairs"] = coerced_pairs
        elif "keyword_pairs" in prepared and prepared.get("keyword_pairs") in (None, "", []):
            prepared.pop("keyword_pairs", None)
        coerced_terms = _coerce_string_list(prepared.get("search_terms"), limit=8, clip_limit=120)
        prepared["search_terms"] = coerced_terms or _default_search_terms(state)
...
def _append_evidence_from_search_results(state: dict, result: dict) -> None:
    result_queries = _dedupe_text_keep_order(result.get("queries", []) or [result.get("query", "")], limit=6, clip_limit=120)
    for item in result.get("results", [])[:6]:
        ...
        evidence.update(_merge_evidence_context(existing, "search_result_list", query_texts=item.get("matched_queries", []) or result_queries, granularity="search_result"))
...
def _merge_evidence_agent_structured_items(state: dict, evidence_result: dict) -> None:
    """将收集证据智能体输出的 stance_reason / source_name 回写到 evidence_by_url。"""
    items = evidence_result.get("structured_evidence_items", [])
    ...
    for item in items:
        if not isinstance(item, dict):
            continue
        url = item.get("url", "")
        if not url or url not in state["evidence_by_url"]:
            continue
        existing = state["evidence_by_url"][url]
        if item.get("stance_reason"):
            existing["stance_reason"] = str(item["stance_reason"])[:300]
        if item.get("source_name"):
            existing["source_name"] = str(item["source_name"])[:100]
        if item.get("stance") and item["stance"] in ("support", "deny", "mixed", "neutral", "irrelevant"):
            existing["agent_stance_hint"] = str(item["stance"])[:20]
...
def _compact_state(state: dict) -> dict:
    ...
    return {
        ...
        "analysis": _compact_tool_result("initial_search_plan", state.get("analysis", {})) if state.get("analysis") else {},
        ...
        "evidence_rounds": state.get("evidence_rounds", [])[-2:],
        "evidence_candidates": [
            {
                "title": item.get("title", ""),
                "url": item.get("url", ""),
...
def _build_structured_evidence_items(
    ...
    evidence_by_url = state.get("evidence_by_url", {})
    ordered: list[dict[str, Any]] = []
    seen_urls: set[str] = set()

    for url in preferred_urls or []:
        evidence = evidence_by_url.get(url)
        if not evidence or url in seen_urls:
            continue
        seen_urls.add(url)
        ordered.append(evidence)

    ranked = sorted(evidence_by_url.values(), key=lambda row: row.get("composite_score", 0), reverse=True)
    ...
    for evidence in ordered[:limit]:
        url = evidence.get("url", "")
        profile = (state.get("source_profiles_by_url") or {}).get(url, {}) or evidence.get("source_profile", {}) or {}
        items.append(
            {
                "url": url,
                "title": evidence.get("title", ""),
                "summary": _clip(evidence.get("summary", ""), 220),
                "stance": _classify_evidence_stance(evidence),
                "source_name": evidence.get("source_name") or profile.get("name") or evidence.get("domain", ""),
                "source_type": evidence.get("source_type", "其他"),
                "domain": evidence.get("domain", ""),
                "tier": profile.get("tier", "unknown"),
                "country": profile.get("country", "未知"),
                "evidence_granularity": evidence.get("evidence_granularity", evidence.get("origin_tool", "search_result")),
                "reason": _clip(evidence.get("reason", ""), 180),
                "stance_reason": _clip(evidence.get("stance_reason", ""), 300),
                "matched_segments": evidence.get("matched_segments", [])[:2],
                "search_queries": evidence.get("search_queries", [])[:3],
                "composite_score": _safe_float(evidence.get("composite_score"), 5.0),
            }
        )
    return items


def _structured_evidence_round_summary(state: dict, round_index: int, agent_result: dict[str, Any] | None = None) -> dict[str, Any]:
    evidence_items = sorted(
        state.get("evidence_by_url", {}).values(),
        key=lambda item: item.get("composite_score", 0),
        reverse=True,
    )
    source_type_counter = Counter(item.get("source_type", "其他") for item in evidence_items)
    source_names = _dedupe_text_keep_order([item.get("source_name", "") for item in evidence_items], limit=6, clip_limit=80)
    top_urls = _top_evidence_urls(state, limit=4)
    structured_evidence_items = _build_structured_evidence_items(
        state,
        limit=6,
        preferred_urls=_dedupe_keep_order((agent_result or {}).get("top_evidence_urls", []) or top_urls),
    )
    round_summary = {
        "round_index": round_index,
        "round_summary": _clip((agent_result or {}).get("round_summary", ""), 300),
        "coverage_assessment": _clip((agent_result or {}).get("coverage_assessment", ""), 220),
        "structured_evidence_ready": bool((agent_result or {}).get("structured_evidence_ready", bool(evidence_items))),
        "top_evidence_urls": _dedupe_keep_order((agent_result or {}).get("top_evidence_urls", []) or top_urls)[:4],
        "structured_evidence_items": structured_evidence_items,
        "key_findings": _coerce_text_list((agent_result or {}).get("key_findings", []), limit=5),
        "remaining_gaps": _coerce_text_list((agent_result or {}).get("remaining_gaps", []), limit=5),
        "recommended_next_queries": _dedupe_text_keep_order((agent_result or {}).get("recommended_next_queries", []), limit=4, clip_limit=120),
        "evidence_count": len(evidence_items),
...
def _filtered_analysis_evidence(state: dict, limit: int = 8) -> list[dict[str, Any]]:
    evidence_by_url = state.get("evidence_by_url", {})
    filtered_items: list[dict[str, Any]] = []
    seen_urls: set[str] = set()

    cross_verify = state.get("cross_verify_result") or {}
    for item in cross_verify.get("traceable_evidence", []) or []:
        if not isinstance(item, dict):
            continue
        url = item.get("url", "")
        if not url or url in seen_urls:
    ...
    for evidence in sorted(evidence_by_url.values(), key=lambda row: row.get("composite_score", 0), reverse=True):
        url = evidence.get("url", "")
        if not url or url in seen_urls:
            continue
        seen_urls.add(url)
...
def _merge_evidence_annotations_to_cross_verify(state: dict, final_result: dict) -> None:
    """将分析智能体输出的 evidence_annotations 合并回 cross_verify_result.traceable_evidence"""
    annotations = final_result.get("evidence_annotations")
    cross_verify = state.get("cross_verify_result")
    if not isinstance(annotations, list) or not isinstance(cross_verify, dict):
        return

    traceable = cross_verify.get("traceable_evidence")
    if not isinstance(traceable, list):
        return

    _STANCE_LABEL = {
        "support": "✅支持", "deny": "❌否认", "mixed": "⚖️双向", "neutral": "➖中立",
    }
    valid_stances = {"support", "deny", "mixed", "neutral"}

    annotation_by_url: dict[str, dict] = {}
    for ann in annotations:
        if isinstance(ann, dict) and ann.get("url"):
            annotation_by_url[ann["url"]] = ann

    for item in traceable:
        url = item.get("url", "")
        ann = annotation_by_url.get(url)
        if not ann:
            continue
        new_stance = str(ann.get("stance", "")).strip().lower()
        if new_stance in valid_stances:
            item["stance"] = new_stance
            item["stance_label"] = _STANCE_LABEL.get(new_stance, "➖中立")
        if ann.get("stance_reason"):
            item["stance_reason"] = str(ann["stance_reason"])[:300]
        if ann.get("source_description"):
            item["source_description"] = str(ann["source_description"])[:200]

    # 同步更新 stance_analysis
    stance_analysis = cross_verify.get("stance_analysis")
    if isinstance(stance_analysis, dict):
        stances = stance_analysis.get("stances", [])
        for stance_item in stances:
            if not isinstance(stance_item, dict):
                continue
            idx = stance_item.get("index")
            matched_trace = next((t for t in traceable if t.get("index") == idx), None)
            if matched_trace:
                stance_item["stance"] = matched_trace.get("stance", stance_item.get("stance", "neutral"))
                stance_item["reason"] = matched_trace.get("stance_reason", stance_item.get("reason", ""))
                stance_item["source_description"] = matched_trace.get("source_description", "")
...
def _llm_json_call(system_prompt: str, payload: dict[str, Any], max_tokens: int = 1800) -> dict[str, Any]:
    ...
    content = response.choices[0].message.content or "{}"
    return _safe_parse_tool_output(content, default={})


def _tool_schemas_for_names(tool_defs: list[dict[str, Any]], allowed_names: list[str]) -> list[dict[str, Any]]:
    allowed_set = set(allowed_names)
    schemas = []
    for tool_def in tool_defs:
        if tool_def.get("name") not in allowed_set:
            continue
        schemas.append(
            {
                "type": "function",
...
def _fallback_supplemental_decision(state: dict) -> dict[str, Any]:
    ...
    return {
        ...
        "selected_evidence_urls": _top_evidence_urls(state, limit=4) if should_stop else [],
...
def _fallback_initial_search_plan(state: dict) -> dict[str, Any]:
    default_query = _normalize_query(state.get("news_text", ""))
    return {
        "analysis_summary": "初步搜索智能体回退到默认搜索计划。",
        "core_question": _clip(state.get("news_text", ""), 120),
        "verification_dimensions": ["来源链路", "核心事实", "关键细节", "传播立场"],
        "focus_points": ["原始发布", "直接证据", "反向核查", "多方来源"],
        "search_queries": [default_query] if default_query else [],
        "risk_hypotheses": ["可能依赖单一来源或二手转述"],
        "img_description": "",
        "missing_information": ["一手来源", "独立印证"],
    }


def _run_initial_search_agent(state: dict) -> dict[str, Any]:
    payload = {
        "news_text": _clip(state.get("news_text", ""), 1000),
        "has_image": bool(state.get("img_base64")),
        "image_path": state.get("image_path"),
        "knowledge_base": _compact_tool_result("knowledge_base_lookup", state.get("knowledge_base", {})) if state.get("knowledge_base") else {},
    }
    fallback = _fallback_initial_search_plan(state)
    try:
        plan = _llm_json_call(INITIAL_SEARCH_AGENT_PROMPT, payload, max_tokens=1400)
    except Exception:
        plan = fallback

    normalized = dict(fallback)
    normalized.update({key: value for key, value in plan.items() if value is not None})
    normalized["focus_points"] = _coerce_text_list(normalized.get("focus_points", []), limit=4) or fallback["focus_points"]
    normalized["verification_dimensions"] = _coerce_text_list(normalized.get("verification_dimensions", []), limit=5) or fallback["verification_dimensions"]
    normalized["risk_hypotheses"] = _coerce_text_list(normalized.get("risk_hypotheses", []), limit=4)
    normalized["missing_information"] = _coerce_text_list(normalized.get("missing_information", []), limit=5)
    normalized["search_queries"] = _dedupe_text_keep_order(normalized.get("search_queries", []), limit=6, clip_limit=120) or fallback["search_queries"]
    normalized["analysis_summary"] = _clip(normalized.get("analysis_summary", fallback["analysis_summary"]), 300)
    normalized["core_question"] = _clip(normalized.get("core_question", fallback["core_question"]), 200)
    normalized["img_description"] = _clip(normalized.get("img_description", ""), 220)
    return normalized


def _run_supplemental_search_agent(state: dict, round_index: int) -> dict[str, Any]:
    latest_round = state.get("evidence_rounds", [])[-1] if state.get("evidence_rounds") else {}
    payload = {
        "round_index": round_index,
        "news_text": _clip(state.get("news_text", ""), 600),
        "analysis": _compact_tool_result("initial_search_plan", state.get("analysis", {})) if state.get("analysis") else {},
        "evidence_round": latest_round,
        "structured_evidence_list": latest_round.get("structured_evidence_items", []),
    ...
    selected_evidence_urls = [url for url in decision.get("selected_evidence_urls", []) if isinstance(url, str) and url.strip()]
    ...
    return {
        ...
        "selected_evidence_urls": _dedupe_keep_order(selected_evidence_urls or fallback["selected_evidence_urls"])[:4],
...
def _run_analysis_agent(state: dict) -> dict[str, Any]:
    filtered_evidence = _filtered_analysis_evidence(state)
    structured_evidence_list = state.get("analysis_structured_evidence") or _build_structured_evidence_items(state)
    payload = {
        "news_text": _clip(state.get("news_text", ""), 800),
        "analysis": _compact_tool_result("initial_search_plan", state.get("analysis", {})) if state.get("analysis") else {},
        "knowledge_base": _compact_tool_result("knowledge_base_lookup", state.get("knowledge_base", {})) if state.get("knowledge_base") else {},
        "cross_verify_result": _compact_tool_result("cross_source_verify", state.get("cross_verify_result", {})) if state.get("cross_verify_result") else {},
        "structured_evidence_list": structured_evidence_list,
        "evidence_candidates": [
            {
                "title": item.get("title", ""),
                "url": item.get("url", ""),
                ...
                "composite_score": _safe_float(item.get("composite_score"), 5.0),
                "evidence_granularity": item.get("evidence_granularity", item.get("origin_tool", "search_result")),
                "stance_reason": _clip(item.get("stance_reason", ""), 300),
            }
...
async def _call_mcp_tool_once(tool_name: str, arguments: dict[str, Any]) -> dict:
    ...
    if not text_output.strip():
        return {}
    return _safe_parse_tool_output(text_output, default={"raw_text": _clip(text_output, 4000)})
...
class MainFactCheckAgent:
    ...
    def _available_tools(self, use_db: bool) -> tuple[list[dict], list[str], list[dict[str, Any]]]:
        tool_defs = self.tools.list_tools()
        schemas, names = _build_runtime_tool_schemas(tool_defs, use_db)
        filtered_defs = [tool_def for tool_def in tool_defs if tool_def.get("name") in set(names)]
        return schemas, names, filtered_defs
    ...
    def _agent_chat_completion(
        ...
        return main_agent_client.chat.completions.create(**kwargs)
    ...
    def _run_tool_calling_agent(
        ...
        tool_schemas = _tool_schemas_for_names(tool_defs, allowed_tool_names)
        messages: list[dict[str, Any]] = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": json.dumps(payload, ensure_ascii=False, indent=2)},
        ]
        ...
        for _ in range(max_steps):
            response = self._agent_chat_completion(messages, tools=tool_schemas, max_tokens=max_tokens)
            response_message = response.choices[0].message
            messages.append(_assistant_message_payload(response_message))
            ...
            if getattr(response_message, "tool_calls", None):
                for tool_call in response_message.tool_calls:
                    ...
                    if tool_name not in set(allowed_tool_names):
                        continue
                    arguments = _safe_parse_tool_output(tool_call.function.arguments or "{}", default={})
                    result = yield from invoke_tool(tool_name, arguments, agent_name=agent_name)
                    messages.append(
                        {
                            "role": "tool",
            ...
            parsed = _safe_parse_tool_output(content, default={"raw_text": _clip(content, 4000)})
    ...
    def run_stream(
        ...
        state = {
            ...
            "evidence_by_url": {},
            "evidence_rounds": [],
            "analysis_structured_evidence": [],
            "source_profiles_by_url": {},
            "history": [],
        }
        _, available_tools, tool_defs = self._available_tools(use_db)
        ...
        analysis_agent_tools = [tool_name for tool_name in ["read_full_page"] if tool_name in available_tools]
        plan = {
            **FOUR_AGENT_PLAN,
            "knowledge_base_gate": "knowledge_base_lookup" if "knowledge_base_lookup" in available_tools else "disabled",
            "initial_search_agent": {"tools": []},
            "evidence_collection_agent": {"tools": evidence_agent_tools},
            "supplemental_search_agent": {"tools": []},
            "analysis_agent": {"tools": analysis_agent_tools},
        ...
        def _invoke_tool(tool_name: str, arguments: dict[str, Any], agent_name: str):
            ...
            yield _evt(
                ...
                tool_source,
                f"{tool_source} 输入",
                detail=json.dumps({"agent": agent_name, "arguments": prepared}, ensure_ascii=False, indent=2),
            )
            ...
            yield _evt(
                ...
                tool_source,
                f"{tool_source} 输出",
                detail=json.dumps({"agent": agent_name, "result": result}, ensure_ascii=False, indent=2),
                data=result,
            )
            return result
        ...
        yield _evt(
            "thinking",
            "system",
            "四个由 main_agent_client 调控的智能体已启动。",
        ...
        if use_db and "knowledge_base_lookup" in available_tools:
            ...
            yield_result = yield from _invoke_tool("knowledge_base_lookup", {"news_text": news_text}, agent_name="initial_search_agent")
            if yield_result is not None:
                final_result = _knowledge_base_direct_result(state)

        if final_result is None:
            state["analysis"] = _run_initial_search_agent(state)
            yield _evt(
                "thinking",
                "initial_search_agent",
                "初步搜索智能体已生成首轮搜索计划。",
                detail=json.dumps(state["analysis"], ensure_ascii=False, indent=2),
            )
            ...
            for round_index in range(1, MAX_SEARCH_ROUNDS + 1):
                ...
                if not round_queries:
                    round_queries = _default_search_queries(state, limit=3)
                if not round_queries or "search_result_list" not in available_tools:
                    break

                evidence_payload = {
                    "round_index": round_index,
                    "news_text": _clip(news_text, 700),
                    "core_question": _clip((state.get("analysis") or {}).get("core_question", ""), 200),
                    "search_queries": round_queries[:3],
                    "structured_evidence_list": state.get("evidence_rounds", [])[-1].get("structured_evidence_items", []) if state.get("evidence_rounds") else [],
                    "structured_context": _compact_state(state),
                    "requirements": {
                        "must_use_tools": True,
                        "preferred_path": ["search_result_list", "extract_relevant_segments"],
                ...
                yield _evt(
                    ...
                    detail=json.dumps(evidence_payload, ensure_ascii=False, indent=2),
                )

                evidence_result = yield from self._run_tool_calling_agent(
                    state,
                    agent_name="evidence_collection_agent",
                    system_prompt=EVIDENCE_COLLECTION_AGENT_PROMPT,
                    payload=evidence_payload,
                    tool_defs=tool_defs,
                    allowed_tool_names=evidence_agent_tools,
                    invoke_tool=_invoke_tool,
                    max_steps=MAX_AGENT_STEPS,
                    max_tokens=2200,
                )
                _merge_evidence_agent_structured_items(state, evidence_result or {})
                state["evidence_rounds"].append(_structured_evidence_round_summary(state, round_index, evidence_result))
                yield _evt(
                    "thinking",
                    "evidence_collection_agent",
                    f"收集证据智能体完成第 {round_index} 轮结构化证据输出。",
                    detail=json.dumps(state["evidence_rounds"][-1], ensure_ascii=False, indent=2),
                )
                ...
                if supplement_decision.get("should_stop") or supplement_decision.get("evidence_sufficient"):
                    state["analysis_structured_evidence"] = _build_structured_evidence_items(
                        state,
                        preferred_urls=supplement_decision.get("selected_evidence_urls", []) or state["evidence_rounds"][-1].get("top_evidence_urls", []),
                    )
                    break
                current_queries = supplement_decision.get("updated_search_queries", []) or [_default_search_query(state)]

            if not state.get("analysis_structured_evidence"):
                state["analysis_structured_evidence"] = _build_structured_evidence_items(state)

            if state.get("evidence_by_url") and "cross_source_verify" in available_tools:
                yield _evt(
                    "thinking",
                    "analysis_agent",
                    "分析智能体已接管，先做交叉验证筛证，再按需补读关键网页。",
                    detail=json.dumps(_compact_state(state), ensure_ascii=False, indent=2),
                )
                yield from _invoke_tool("cross_source_verify", {}, agent_name="analysis_agent")

            analysis_payload = {
                "news_text": _clip(news_text, 800),
                "core_question": _clip((state.get("analysis") or {}).get("core_question", ""), 200),
                "filtered_evidence_candidates": [
                    {
                        "title": item.get("title", ""),
                        "url": item.get("url", ""),
                    ...
                    for item in _filtered_analysis_evidence(state)
                ],
                "cross_verify_result": _compact_tool_result("cross_source_verify", state.get("cross_verify_result", {})) if state.get("cross_verify_result") else {},
                "structured_evidence_list": state.get("analysis_structured_evidence") or _build_structured_evidence_items(state),
                "structured_context": _compact_state(state),
            ...
            yield _evt(
                ...
                detail=json.dumps(analysis_payload, ensure_ascii=False, indent=2),
            )

            final_result = yield from self._run_tool_calling_agent(
                state,
                agent_name="analysis_agent",
                system_prompt=ANALYSIS_AGENT_PROMPT,
                payload=analysis_payload,
                tool_defs=tool_defs,
                allowed_tool_names=analysis_agent_tools,
                invoke_tool=_invoke_tool,
                max_steps=max(4, MAX_SEARCH_ROUNDS + 2),
                max_tokens=2400,
            )
            final_result = _normalize_final_result(final_result or _run_analysis_agent(state), state)
        ...
        final = {
            ...
            "plan": plan,
            "cross_verify_result_for_display": _compact_tool_result("cross_source_verify", state.get("cross_verify_result", {})) if state.get("cross_verify_result") else None,
        }
        yield _evt("final", "analysis_agent", "最终判断结果", data=final)
        yield _evt(
            "db_status",
            "system",
            "数据库写入成功",
