...
INITIAL_SEARCH_AGENT_PROMPT = """你是初步搜索智能体。你的职责是结合原始声明与新闻知识库参考，生成首轮搜索计划。
...
- search_queries: 建议交给收集证据智能体执行的搜索项。
...
你可用的工具只有四个：
- search_result_list: 输如执行搜索
- extract_relevant_segments: 读取网页部分文本
- read_full_page: 读取网页全文
...
def _prepare_tool_arguments(state: dict, tool_name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if tool_name == "knowledge_base_lookup":
        ...
        single_query = _normalize_query(prepared.get("query", ""))
        if single_query:
        ...
        prepared["query"] = prepared["queries"][0] if prepared["queries"] else ""
...
def _initial_agent_messages(state: dict) -> list[dict[str, Any]]:
    ...
    return [
        {"role": "system", "content": INITIAL_SEARCH_AGENT_PROMPT},
