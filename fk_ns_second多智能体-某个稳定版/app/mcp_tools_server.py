...
def search_result_list(
    ...
    """
    return tool_c1_search_results(query=query, queries=queries, max_results=max_results, exclude_urls=exclude_urls)
...
def finalize_and_store(
    ...
    - news_text: 新闻文本。
    - image_path: 图片路径。
    - img_description: 图片说明。
    - analysis_result: 最终分析结果。
    - iteration_logs: 多轮执行日志。
    - all_evidence_items: 全部证据项。
    - cross_verify_result: 可选，交叉验证结果。

    输出:
    - classification: 最终评分。
    - reason: 最终结论说明。
    - evidence_url: 关键证据链接。
    - db_updated/new_id: 数据库存储结果。
    """
